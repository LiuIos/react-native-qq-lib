...
function translateError(err, result) {
  ...
  if (typeof err === "object") {
    ...
    return this.reject(
      Object.assign(new Error(err.message), { errCode: err.errCode })
    );
...
export function shareToQQ(data = {}) {
  return QQAPI.shareToQQ(data).then(() => waitForResponse("QQShareResponse"));
}

export function shareToQzone(data = {}) {
  return QQAPI.shareToQzone(data).then(() =>
    waitForResponse("QQShareResponse")
  );
}
...
export function init(universalLink = "") {
  if (Platform.OS === "ios") {
    QQAPI.initApi(universalLink);
  } else {
    QQAPI.init();
  }
}
